...
如之前用到的监控工具cacti就是通过snmp来获取系统的一些指标，然后通过rrdtool来绘图


## 监控的重要性

故障层面：业务不能访问，只要任何一个环节有问题，就会整个都出问题

性能层面：哪个时间段怎么样，当前情况怎么样，预测未来。
...
监控的指标：通过看服务商监控了哪些东西来监控（www.jiankongbao.com）
...
1. 使用IPMI
2. 机房巡检（一个月或一个季度都去查看一下，跟机房工程师交流一下）
...
> 所有的远程控制卡都会根据IPMI这个标准来做的，所以我们可以使用IPMI这个工具来对这些服务器做监控；IPMI依赖一个BMC的微控制器，通过这个就可以监控一些硬件的指标，如温度、磁盘故障等；它也是插在PCI/E插槽的，后期可加，一块六百左右，有两个LICENSE，**重听**
...
SNMP发布了三个版本，一般可能会使用第二个版本，第三个版本加入了身份验证的功能
